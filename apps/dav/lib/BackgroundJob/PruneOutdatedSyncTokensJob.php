...
class PruneOutdatedSyncTokensJob extends TimedJob {
	...
	public function __construct(CalDavBackend $calDavBackend, CardDavBackend $cardDavBackend, IConfig $config, LoggerInterface $logger) {
	...
	public function run($argument) {
		$limit = (int) $this->config->getAppValue(Application::APP_ID, 'totalNumberOfSyncTokensToKeep', '10000');

		// 0 or less is more likely a misconfiguration from the app config than an actual value.
		if ($limit <= 0) {
			$limit = 10000;
		}
